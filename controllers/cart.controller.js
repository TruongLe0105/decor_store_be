...
controllerCart.addProductToCart = catchAsync(async (req, res, next) => {
    ...
    if (!cart) {
        cart = await Cart.create({
            ...
            user: currentUserId,
            products: [
                { _id: productId, name, price, categories, imageUrl, quantity: (quantity > 0 && quantity <= LIMIT_PRODUCT_CAN_BUY) ? quantity : 1 }
            ],
            totalPrice: price,
        })
    } else {
        ...
        if (productCart) {
            ...
            cart.products.push({
                _id: productId, name, categories, price, imageUrl, quantity: quantity > 0 ? quantity : 1
            });
        }
